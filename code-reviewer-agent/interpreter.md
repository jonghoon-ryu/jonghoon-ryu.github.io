...
- Expr(표현식): 실행하면 값 하나로 평가(evaluate)되는 단위. Expr과 Token을 조합해서
  만든다.
- Stmt(문장): 값 반환 없이 동작을 수행하는 단위. Expr, Stmt, Token을 조합해서
  만든다.
- 제약: Expr 내부의 자식으로 Stmt를 가질 수 없다 (Stmt는 Expr을 가질 수 있지만 역은
  불가). 트리의 루트는 항상 Stmt.
- Token 자체는 노드가 아니라 각 노드의 필드로만 보관된다(예: =, ; 같은 실행에
  불필요한 토큰은 트리에 노드로 남지 않음).
