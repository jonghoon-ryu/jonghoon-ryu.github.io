...
이 경쟁이 실제로 터지려면 "GC 가 자기 마이그레이션으로 방금 채운 block 을, 그 물리적 쓰기가 아직 끝나기도 전에 자기 자신의 다음 청소 대상으로 다시 뽑는" 상황이 벌어져야 한다. 실제 MQSim 이 가정하는 수천 개 block 규모에서는 이런 우연이 사실상 일어나지 않는다 — 이번에도 이 프로젝트의 작은 데모 규모(블록 수 최소 8개)에서만 현실적인 확률로 드러났다.
...
원인: GC 블록 선택 정책(RGA)의 후보 탐색 루프에 **애초에 반복 횟수 제한이 없었다**:
