...
나머지 3개("매핑 기본" + block 당 page 4, block 8 + page 4, block 8 + OP 0)는 이전 엔진과 결과가 완전히 같았고, 원인도 버그가 아니었다. 종료 시점에 invalid page 가 **하나도 없다** — 이 프리셋은 논리 공간 100% 에 쓰기 때문에, 용량이 작으면 덮어쓰기가 일어나기도 전에 장치가 가득 차서 쓰기가 막힌다. 청소할 게 없으니 GC 도 할 일이 없다. 이건 이미 `DEFAULT_MAPPING_PARAMS` 주석에 "장치가 차면 데모가 거기서 끝난다"고 기록해둔 설계상 한계다. 다만 화면에서는 재생이 이유 없이 멈추는 것처럼 보였기 때문에, 실행이 끝났는데 빈 page 를 기다리는 쓰기가 남아 있으면 통계 열에 **"장치가 가득 찼어요"** 안내와 늘려볼 파라미터를 보여주도록 했다(앱 커밋 `a24a925`). 슬라이더 범위는 그대로 뒀다 — 장치가 가득 차는 것도 실제 FTL 동작이라, 막기보다 보여주는 쪽을 택했다.
