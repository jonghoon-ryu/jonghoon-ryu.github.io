...
- 각 세션은 [MQSim 개괄](/ftl-visual-simulator/reference/mqsim/code-analysis/overview/) 문서의 해당 부분을 먼저 훑고, 실제 소스 파일을 열어 문서의 설명과 코드가 정말 일치하는지 확인하는 방식으로 진행
...
- `TSU.Schedule()` 이 여러 채널/칩 중 무엇을 언제 실행시킬지 정하는 기준 — 실제로는 채널별 라운드로빈 + 칩 하나당 read>write>erase 우선순위( `TSU_Priority_OutOfOrder::Schedule()` 실제 코드로 확인, [코드 분석 7-4절](/ftl-visual-simulator/reference/mqsim/code-analysis/overview/) 참고 )
...
- 관련 문서 : [개발 계획](/ftl-visual-simulator/plan/) · [MQSim 개요](/ftl-visual-simulator/reference/mqsim/overview/) · [MQSim 코드 분석](/ftl-visual-simulator/reference/mqsim/code-analysis/) · [FTL 개념 ↔ 파라미터·모듈 대응](/ftl-visual-simulator/reference/mqsim/code-analysis/concept-mapping/)
