...
> ⚠️ **( 9/6 기록 ) static WL 은 완료, dynamic WL 은 아직.** 버그 수정([PR #5](https://github.com/jonghoon-ryu/ftl-visual-simulator/pull/5))과 hook 추가([PR #7](https://github.com/jonghoon-ryu/ftl-visual-simulator/pull/7))를 별도 PR 로 분리했다. static WL 검증 중 원본 MQSim 버그 2개(`run_static_wearleveling()`의 블록 주소/ID 혼동, `Get_min_max_erase_difference()`의 인덱스-차 vs erase-count-차 혼동)를 발견해 고쳤다 — 상세 내용과 "이건 upstream 과 의도적으로 다르게 동작하기로 한 결정"이라는 점은 [마모 평준화 버그와 의도적 동작 변경](/ftl-visual-simulator/reference/wl-bug-deviation/) 참고. dynamic WL 은 `PlaneBookKeepingType`( plane 좌표를 모르는 클래스 )에 있어서, 이벤트에 필요한 `Physical_Page_Address` 를 만들려면 호출부(`Flash_Block_Manager_Page_Level.cpp` 쪽)까지 같이 봐야 해서 다음으로 미룸.

- `PlaneBookKeepingType::Add_to_free_block_pool()` / `Get_a_free_block()` : dynamic WL 로 인해 어떤 블록이 다음 write frontier 로 선택됐는지 이벤트( erase count 최소 블록 우선 배정 ) — **아직( 위 기록 참고 )**
