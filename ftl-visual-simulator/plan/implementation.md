...
> ⚠️ **( 9/6 기록 ) static WL 은 완료, dynamic WL 은 아직.** static WL 검증 중 원본 MQSim 버그 2개(`run_static_wearleveling()`의 블록 주소/ID 혼동, `Get_min_max_erase_difference()`의 인덱스-차 vs erase-count-차 혼동)를 발견해 고쳤다. dynamic WL 은 `PlaneBookKeepingType`( plane 좌표를 모르는 클래스 )에 있어서, 이벤트에 필요한 `Physical_Page_Address` 를 만들려면 호출부(`Flash_Block_Manager_Page_Level.cpp` 쪽)까지 같이 봐야 해서 다음으로 미룸.
