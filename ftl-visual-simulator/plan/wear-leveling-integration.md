...
- **마모평준화 임계값 슬라이더** (이 프리셋에서만, 1-5): 임계값 1-4 에서 WL 이 341/23/7/5 번, 5 이상은 이 실행 길이 안에서 0 번이라 6-10 은 뺐다. 5 는 "여기서는 발동 안 함"을 보여주는 값으로 남김.
- **block 별 "데이터" 열**: cold(한 번 쓰고 안 건드리는 flow) / hot(계속 덮어쓰는 flow) / -(데이터 없음). 막대의 🔥/❄️ 는 마모 정도라서, 헷갈리지 않게 다른 색 태그와 설명 한 줄을 따로 뒀다. 실행 중간에 보면 cold block 들은 erase 0 에 머물러 있다.
- (다른 프리셋) "매핑 기본"을 아주 작은 용량으로 돌리면 장치가 가득 차서 멈추는데, 그때 **"장치가 가득 찼어요"** 안내를 띄우도록 했다.
